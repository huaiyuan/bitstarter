...
var TMPFILE_DEFAULT = "/tmp/index.html";
...
var checkHtmlFile = function(htmlfile, checksfile) {
    return checkHtmlBuffer(fs.readFileSync(htmlfile), checksfile);
};
...
var checkShowResult = function(checkJson) {
    ...
    if (program.url) {
        rest.get(program.url).on('complete', function(result, response) {
            if (result instanceof Error) {
                ...
                checkShowResult(checkHtmlBuffer(result, program.checks));
            }
        });
    } else {
        checkShowResult(checkHtmlFile(program.file, program.checks));
    }
} else {
